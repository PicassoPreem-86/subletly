...
export default function EditPropertyPage() {
  ...
  const fetchProperty = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error(property.error || 'Failed to fetch property');
      }

      // Verify ownership
      if (property.landlordId !== session?.user?.id) {
        setFetchError('You do not have permission to edit this property');
        return;
      }
      ...
      setFormData({
        ...
        minStay: property.minStay?.toString() || '',
        amenities: property.amenities || [],
        images: property.images || [],
        petsAllowed: property.petsAllowed || false,
        smoking: property.smoking || false,
        parking: property.parking || false,
        status: property.status || 'DRAFT',
      });
    } catch (error) {
      setFetchError(error instanceof Error ? error.message : 'Failed to load property');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  if (!formData || !session?.user || session.user.accountType !== 'LANDLORD') {
    return null;
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-12">
        ...
        {/* Form - Same structure as create page but with pre-populated data */}
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          {/* All other sections would be identical to the create page, just pre-populated */}
          {/* For brevity, I'll include the key sections but the pattern is the same */}
          ...
          <div className="bg-white rounded-xl shadow-sm p-8">
            ...
            <div className="flex gap-4">
              ...
              <button
                type="button"
                onClick={() => setFormData((prev) => prev ? ({ ...prev, status: 'ACTIVE' }) : null)}
                className={`flex-1 px-6 py-4 rounded-lg border-2 font-medium transition-all ${
